...
<?php
	if(isset($_REQUEST['active'])) {
	  ...
	  // echo $id;
		...
		$sql="UPDATE user_registration SET active='$active' WHERE user_registration_id='$id'";
		
			if(mysql_query($sql)) {

				header("location: buyer_active.php?msg=Active Successfully ");
...
if(isset($_REQUEST['inactive'])) {
		...
		$sql = "UPDATE user_registration SET active='$inactive' WHERE user_registration_id='$id'";
		
			if(mysql_query($sql)) {

				header("location: buyer_active.php?msg=Inactive Successfully ");
								...
								<?php 
									...
									if($query_run = mysql_query($query)) {
										...
										while($query_rows = mysql_fetch_assoc($query_run)) {
											$id=$query_rows['user_registration_id'];
											$buyer_email=$query_rows['user_registration_email'];
											$buyer_name = $query_rows['user_registration_name'];
											$buyer_contact = $query_rows['user_registration_contact_no'];
											$buyer_active = $query_rows['active'];
											
                                              if($buyer_active=='1'){

                                                 echo '<tr style="text-align:center">';
											
											echo '<td>'.$buyer_name.'</td><td>'.$buyer_email.'</td><td>'.$buyer_contact.'</td><td><a href="buyer_active.php?inactive='.$id.'">Inactive</a></td>';
											echo '</tr>';
											...
											echo '<td>'.$buyer_name.'</td><td>'.$buyer_email.'</td><td>'.$buyer_contact.'</td><td><a href="buyer_active.php?active='.$id.'">Active</a></td>';
											echo '</tr>';
											   }
