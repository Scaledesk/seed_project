		                    	...
		                    	<div class="col-sm-12">
		                    		<div class="blog-post">
										...
										if(isset($_GET['update_id'])) {
											...
											$sql = "SELECT * FROM resources WHERE id='$update_id'";
												if($sql_run = mysql_query($sql)) {
													while($sql_rows = mysql_fetch_assoc($sql_run)) {
														$title = $sql_rows['title'];
														$detail = $sql_rows['detail'];
														echo '<div class="col-md-6 col-sm-6">';
														echo '<h2 style="margin-bottom:20px;">Edit Resource</h2>';
														echo '<form method="POST" action="update-resource.php">';
														echo 'Enter title:<input type="text" name="update_title" value="'.$title.'">';
														echo 'Enter Resource Detail :<textarea name="detail" style="width:100%;">'.$detail.'</textarea>';
														echo '<input type="hidden" name="up_id" value="'.$update_id.'">';
														echo '<input type="submit" name="update" value="Update" class="btn btn-accent">';
														echo '</from>';
														echo '</div>';
													}
												}
										...
										<div class="col-sm-12">
										<div class="col-sm-12">
										<h1 style="margin-bottom:20px;">Resource Detail</h1>
										<table border="1" style="width:100%;">
										<th style="text-align:center;">Resource Title</th>
										<th style="text-align:center;">Resource Detail</th>
										...
										<?php
											...
											if($query_run = mysql_query($query)) {
												while($query_rows = mysql_fetch_assoc($query_run)) {
													...
													$detail = $query_rows['detail'];
													
													echo '<tr style="text-align:center;"><td>'.$title.'</td><td>'.$detail.'</td><td><a href="update-resource.php?update_id='.$id.'">Update</a></td></tr>'; 
